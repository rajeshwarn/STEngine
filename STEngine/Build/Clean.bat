...
:ReadyToCompile
if not exist Programs\STBuildTool\STBuildTool.csproj goto ReadyToClean
msbuild /nologo /verbosity:quiet Programs\STBuildTool\STBuildTool.csproj /property:Configuration=Development /property:Platform=AnyCPU
if not %ERRORLEVEL% == 0 goto Error_UBTCompileFailed

:ReadyToClean
IF EXIST ..\..\Engine\Binaries\DotNET\STBuildTool.exe (
         ..\..\Engine\Binaries\DotNET\STBuildTool.exe %* -clean
) ELSE (
