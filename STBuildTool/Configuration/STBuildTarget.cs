...
namespace STBuildTool
{
    ...
    public class STBuildConfig
    {
        ...
        public STTargetConfiguration Config;
    ...
    /// <summary>
    /// Describes all of the information needed to initialize a STBuildTarget object
    /// </summary>
    ...
    public class STBuildTarget
    {
        ...
        public static STBuildTarget CreateTarget(TargetDescriptor Desc)
        {
            ...
            STBuildTarget BuildTarget = null;
            if (!ProjectFileGenerator.bGenerateProjectFiles)
            {
                ...
                RulesCompiler.SetAssemblyNameAndGameFolders(PossibleAssemblyName, STBuildTarget.DiscoverAllGameFolders());
            }
        ...
        public string OutputPath
        {
            get
            {
                ...
                {
                    throw new BuildException("Attempted to use STBuildTarget.OutputPath property, but there are multiple (or no) OutputPaths. You need to handle multiple in the code that called this (size = {0})", OutputPaths.Length);
                }
        ...
        [NonSerialized]
        public List<STBuildBinary> AppBinaries = new List<STBuildBinary>();
        ...
        [NonSerialized]
        private Dictionary<string, STBuildModule> Modules = new Dictionary<string, STBuildModule>(StringComparer.InvariantCultureIgnoreCase);
        ...
        private void CleanUnrealHeaderTool()
        {
            if (!STBuildTool.RunningRocket())
            {
                ...
                ExternalExecution.RunExternalExecutable(STBuildTool.GetUBTPath(), UBTArguments.ToString());
            }
        }
        ...
        public void GenerateExternalFileList()
        {
            ...
            foreach (STBuildBinary Binary in AppBinaries)
            {
                foreach (STBuildModule Module in Binary.GetAllDependencyModules(bIncludeDynamicallyLoaded: false, bForceCircular: false))
                {
                    STBuildExternalModule ExternalModule = Module as STBuildExternalModule;
            ...
            ISTBuildPlatform BuildPlatform = STBuildPlatform.GetBuildPlatform(Platform);
            ...
            foreach (string ModuleName in ModuleNames)
            {
                ...
                string LibraryExtension = BuildPlatform.GetBinaryExtension(STBuildBinaryType.StaticLibrary);
            ...
            if (STBuildConfiguration.bMergeExternalFileList)
            {
        ...
        public ECompilationResult Build(ISTToolChain TargetToolChain, out List<FileItem> OutputItems, out List<SHTModuleInfo> UObjectModules, out string EULAViolationWarning)
        {
            OutputItems = new List<FileItem>();
            UObjectModules = new List<SHTModuleInfo>();
            ...
            if (ShouldCompileMonolithic())
            {
                ...
                {
                    IsCurrentPlatform = Platform == STTargetPlatform.Mac;
                }
                else
                {
                    IsCurrentPlatform = Platform == STTargetPlatform.Win64 || Platform == STTargetPlatform.Win32;
                    ...
                    && IsCurrentPlatform)
                {
                    ...
                    if (Platform == STTargetPlatform.Mac && OutputFilePath.Contains(".app/Contents/MacOS"))
                    {
                ...
                if (STBuildTool.HasUProjectFile())
                {
                    string ProjectName = Path.GetFileNameWithoutExtension(STBuildTool.GetUProjectFile());
                ...
                if ((STBuildTool.BuildingRocket() || STBuildTool.RunningRocket()) && TargetRules.IsAGame(TargetType))
                {
                    List<STBuildModule> Modules = ExecutableBinary.GetAllDependencyModules(true, false);
                    foreach (STBuildModuleCPP Module in Modules.OfType<STBuildModuleCPP>())
                    {
                        ...
                        {
                            STTargetConfiguration LibraryConfiguration = (Configuration == STTargetConfiguration.DebugGame) ? STTargetConfiguration.Development : Configuration;
                            Module.RedistStaticLibraryPaths = MakeBinaryPaths("", "UE4Game-Redist-" + Module.Name, Platform, LibraryConfiguration, STBuildBinaryType.StaticLibrary, TargetType, null, AppName);
                            Module.bBuildingRedistStaticLibrary = STBuildTool.BuildingRocket();
            ...
            if (STBuildConfiguration.bGenerateExternalFileList)
            {
            ...
            if (STBuildConfiguration.bGenerateManifest || STBuildConfiguration.bCleanProject)
            {
            ...
            if ((BuildConfiguration.bXGEExport && STBuildConfiguration.bGenerateManifest) || (!STBuildConfiguration.bGenerateManifest && !STBuildConfiguration.bCleanProject && !ProjectFileGenerator.bGenerateProjectFiles))
            {
                ...
                foreach (var Binary in AppBinaries)
                {
                    ...
                    foreach (var DependencyModuleCPP in DependencyModules.OfType<STBuildModuleCPP>().Where(CPPModule => !LocalUObjectModules.Any(Module => Module.ModuleName == CPPModule.Name)))
                    {
                        ...
                        if (UHTModuleInfo.PublicUObjectClassesHeaders.Count > 0 || UHTModuleInfo.PrivateUObjectHeaders.Count > 0 || UHTModuleInfo.PublicUObjectHeaders.Count > 0)
                        {
                            ...
                            STBuildModuleCPP.AutoGenerateCppInfoClass.BuildInfoClass BuildInfo = null;
                            UHTModuleInfo.GeneratedCPPFilenameBase = Path.Combine(STBuildModuleCPP.GetGeneratedCodeDirectoryForModule(this, UHTModuleInfo.ModuleDirectory, UHTModuleInfo.ModuleName), UHTModuleInfo.ModuleName) + ".generated";
                            if (DependencyModuleCPP.SourceFilesToBuild.Count != 0)
                            {
                                BuildInfo = new STBuildModuleCPP.AutoGenerateCppInfoClass.BuildInfoClass(UHTModuleInfo.GeneratedCPPFilenameBase + "*.cpp");
                            }

                            DependencyModuleCPP.AutoGenerateCppInfo = new STBuildModuleCPP.AutoGenerateCppInfoClass(BuildInfo);
                            ...
                            UHTModuleInfo.PCH = "";
                            if (STBuildTool.IsGatheringBuild && !STBuildTool.IsAssemblingBuild)
                            {
        ...
        public List<string> PreBuildSetup(ISTToolChain TargetToolChain)
        {
            ...
            var NewBinaries = new List<STBuildBinary>();
            foreach (var Binary in AppBinaries)
            {
            ...
            else if (STBuildConfiguration.bHotReloadFromIDE)
            {
            ...
            {
                var FilteredBinaries = new List<STBuildBinary>();
            ...
            if (STBuildTool.GetOnlyPlatformSpecificFor() != STTargetPlatform.Unknown)
            {
                var FilteredBinaries = new List<STBuildBinary>();
            ...
            if (STBuildTool.RunningRocket() && AppName != "UnrealHeaderTool")
            {
                var FilteredBinaries = new List<STBuildBinary>();

                // Have to do absolute here as this could be a project that is under the root
                string FullUProjectPath = Path.GetFullPath(STBuildTool.GetUProjectPath());
        ...
        private List<STBuildBinary> FilterOnlyModules()
        {
            var FilteredBinaries = new List<STBuildBinary>();
        ...
        private List<STBuildBinary> FilterGameModules()
        {
            var FilteredBinaries = new List<STBuildBinary>();
        ...
        private void CreateLinkerFixupsCPPFile()
        {
            ...
            STBuildBinaryCPP BinaryCPP = ExecutableBinary as STBuildBinaryCPP;
            if (BinaryCPP != null)
            {
            ...
            // We ALWAYS have to write this file as the IMPLEMENT_PRIMARY_GAME_MODULE function depends on the UELinkerFixups function existing.
            {
                ...
                // Include an empty header so STBuildModule.Compile does not complain about a lack of PCH
        ...
        private STBuildModuleCPP CreateArtificialModule(string Name, string Directory, IEnumerable<FileItem> SourceFiles, IEnumerable<string> PrivateDependencyModuleNames)
        {
            return new STBuildModuleCPP(
                InTarget: this,
                InName: Name,
                InType: STBuildModuleType.Game,
                ...
                InPublicAdditionalBundleResources: new List<STBuildBundleResource>(),
                InPublicAdditionalFrameworks: new List<STBuildFramework>(),
        ...
        private List<SharedPCHHeaderInfo> FindSharedPCHHeaders()
        {
            ...
            foreach (var Binary in AppBinaries)
            {
                var CPPBinary = Binary as STBuildBinaryCPP;
                if (CPPBinary != null)
                {
                    foreach (var ModuleName in CPPBinary.ModuleNames)
                    {
                        var CPPModule = GetModuleByName(ModuleName) as STBuildModuleCPP;
                        if (CPPModule != null)
                        {
                            if (!String.IsNullOrEmpty(CPPModule.SharedPCHHeaderFile))
                            {
                                ...
                                {
                                    var ModuleDependencies = new Dictionary<string, STBuildModule>();
                                    var ModuleList = new List<STBuildModule>();
                                    bool bIncludeDynamicallyLoaded = false;
        ...
        protected void AddExtraModule(string ModuleName)
        {
            ...
            else
            {
                ...
                string[] OutputFilePaths = MakeBinaryPaths(ModuleName, GetAppName() + "-" + ModuleName, STBuildBinaryType.DynamicLinkLibrary, TargetType, null, AppName);
        ...
        protected virtual void SetupPlugins()
        {
            ...
            if (TargetType != TargetRules.TargetType.Program && STBuildTool.HasUProjectFile())
            {
                ...
                EnabledPluginNames = UProjectInfo.GetEnabledPlugins(STBuildTool.GetUProjectFile(), EnabledPluginNames, Platform);
            }
            ...
            if (STBuildTool.BuildingRocket() && TargetType != TargetRules.TargetType.Program)
            {
        ...
        protected virtual void SetupBinaries()
        {
            if (Rules != null)
            {
                List<STBuildBinaryConfiguration> RulesBuildBinaryConfigurations = new List<STBuildBinaryConfiguration>();
                ...
                foreach (STBuildBinaryConfiguration BinaryConfig in RulesBuildBinaryConfigurations)
                {
                    ...
                    {
                        AppBinaries.Add(new STBuildBinaryCPP(this, BinaryConfig));
                    }
                    else
                    {
                        AppBinaries.Add(new STBuildBinaryCSDLL(this, BinaryConfig));
                    }
                }
        ...
        public virtual void SetupGlobalEnvironment()
        {
            ...
            var ToolChain = STToolChain.GetPlatformToolChain(BuildPlatform.GetCPPTargetPlatform(Platform));
            ...
            if (Rules != null)
            {
                ...
                if (bAllowGameOverride ||
                    (Rules.ToString() == "UE4EditorTarget" && STBuildTool.BuildingRocket()) ||	// @todo Rocket: Hard coded target name hack
            ...
            BuildPlatform.ValidateSTBuildConfiguration();
            STBuildConfiguration.ValidateConfiguration();
            ...
            if (STBuildTool.BuildingRocket() || STBuildTool.RunningRocket())
            {
            ...
            if (STBuildTool.RunningRocket() || (STBuildTool.HasUProjectFile() && ShouldCompileMonolithic()))
            {
                ...
                if (STBuildTool.RunningRocket())
                {
                    ...
                    if (IntermediateConfiguration != STTargetConfiguration.Development && IntermediateConfiguration != STTargetConfiguration.Shipping)
                    {
                        IntermediateConfiguration = STTargetConfiguration.Development;
                    }
                }
                ...
                if (ShouldCompileMonolithic())
                {
                    GlobalCompileEnvironment.Config.OutputDirectory = Path.Combine(STBuildTool.GetUProjectPath(), BuildConfiguration.PlatformIntermediateFolder, OutputAppName, IntermediateConfiguration.ToString());
                }
            }
            else
            {
                var GlobalEnvironmentConfiguration = Configuration == STTargetConfiguration.DebugGame ? STTargetConfiguration.Development : Configuration;
            ...
            if (STBuildConfiguration.bCompileAgainstEngine)
            {
                ...
                GlobalCompileEnvironment.Config.Definitions.Add(
                    String.Format("WITH_UNREAL_DEVELOPER_TOOLS={0}", STBuildConfiguration.bBuildDeveloperTools ? "1" : "0"));
                // Automatically include CoreUObject
                STBuildConfiguration.bCompileAgainstCoreUObject = true;
            }
            else
            {
            ...
            if (STBuildConfiguration.bCompileAgainstCoreUObject)
            {
            ...
            if (STBuildConfiguration.bCompileWithStatsWithoutEngine)
            {
            ...
            if (STBuildConfiguration.bCompileWithPluginSupport)
            {
            ...
            if (STBuildConfiguration.bUseLoggingInShipping)
            {
            ...
            if (STBuildConfiguration.bCompileLeanAndMeanUE)
            {
            ...
            if (STBuildConfiguration.bBuildEditor)
            {
        ...
        {
            STBuildPlatform.GetBuildPlatform(Platform).SetUpConfigurationEnvironment(this);
        ...
        public STBuildModule FindOrCreateModuleByName(string ModuleName)
        {
            STBuildModule Module;
            if (!Modules.TryGetValue(ModuleName, out Module))
            {
                ...
                var ModuleType = STBuildModuleType.Unknown;
                ...
                {
                    ModuleType = STBuildModuleType.ThirdParty;
                }
                else
                {
                    // Check if it's a plugin
                    ModuleType = STBuildModule.GetPluginModuleType(ModuleName);
                    if (ModuleType == STBuildModuleType.Unknown)
                    {
                        ...
                        if (ModuleFileRelativeToEngineDirectory.StartsWith("..") || Path.IsPathRooted(ModuleFileRelativeToEngineDirectory))
                        {
                            ...
                            if (SourceIndex != -1)
                            {
                                ...
                                ModuleType = STBuildModuleType.Game;
                            }
                            else
                            {
                        ...
                        {
                            ModuleType = STBuildModule.GetEngineModuleTypeBasedOnLocation(ModuleName, ModuleType, ModuleFileRelativeToEngineDirectory);
                            if (ModuleType == STBuildModuleType.Unknown)
                            {
                ...
                string ProjectSourcePath = STBuildTool.GetUProjectSourcePath();
                ...
                if (RulesObject.Type == ModuleRules.ModuleType.CPlusPlus || RulesObject.Type == ModuleRules.ModuleType.CPlusPlusCLR)
                {
                    ...
                    if (bDiscoverFiles && (!STBuildTool.RunningRocket() || Utils.IsFileUnderDirectory(ModuleFileName, STBuildTool.GetUProjectPath())))
                    {
                ...
                STTargetPlatform Only = STBuildTool.GetOnlyPlatformSpecificFor();

                if (Only == STTargetPlatform.Unknown && STBuildTool.SkipNonHostPlatforms())
                {
                    Only = Platform;
                }
                ...
                // locations.
                STBuildPlatform.PlatformModifyNewlyLoadedModule(Module, TargetInfo, Only);
            }
            return Module;
        }
        ...
        /** Finds a module given its name.  Throws an exception if the module couldn't be found. */
        public STBuildModule GetModuleByName(string Name)
        {
            STBuildModule Result;
        ...
        static List<FileItem> GetCPlusPlusFilesToBuild(List<string> SourceFiles, string SourceFilesBaseDirectory, STTargetPlatform TargetPlatform)
        {
