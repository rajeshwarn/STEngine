...
namespace STBuildTool.IOS
{
	public class UEDeployIOS : STBuildDeploy
	{
		/**
		...
		public override void RegisterBuildDeploy()
		{
			...
			STBuildDeploy.RegisterBuildDeploy(STTargetPlatform.IOS, this);
		}
...
//			}
		...
		public override bool PrepTargetForDeployment(STBuildTarget InTarget)
		{
			...
			if (InTarget.Configuration == STTargetConfiguration.Development)
			{
			...
			if (BuildHostPlatform.Current.Platform == STTargetPlatform.Mac && Environment.GetEnvironmentVariable("UBT_NO_POST_DEPLOY") != "true")
			{
			...
			else
			{
				...
				GeneratePList(ProjectDirectory, bIsUE4Game, GameName, Path.GetFileNameWithoutExtension(STBuildTool.GetUProjectFile()), "../../Engine", "");
			}
			return true;
		}
