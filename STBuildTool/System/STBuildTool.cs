...
{
    public partial class STBuildTool
    {
        ...
        static STTargetPlatform OnlyPlatformSpecificFor = STTargetPlatform.Unknown;
        ...
        static public bool IsGatheringBuild
        {
            get
            {
                ...
                {
                    throw new BuildException("Accessed STBuildTool.IsGatheringBuild before it was fully initialized at startup (bIsSafeToCheckIfGatheringOrAssemblingBuild)");
                }
            }
        }
        ...
        /// <summary>
        /// Returns true if STBuildTool should skip everything other than the host platform as far as finding other modules that need to be compiled.
        /// </summary>
        ...
        static public STTargetPlatform GetOnlyPlatformSpecificFor()
        {
            return OnlyPlatformSpecificFor;
        }

        /// <summary>
        /// Returns true if STBuildTool is running with the "-rocket" option (Rocket mode)
        /// </summary>
        ...
        {
            string STBuildToolPath = Path.Combine(Utils.GetExecutingAssemblyDirectory(), "STBuildTool.exe");
            return STBuildToolPath;
        }
        ...
        static public bool IsDesktopPlatform(STTargetPlatform InPlatform)
        {
        ...
        static public bool IsEditorPlatform(STTargetPlatform InPlatform)
        {
        ...
        static public bool IsServerPlatform(STTargetPlatform InPlatform)
        {
        ...
        static public bool PlatformSupportsCrashReporter(STTargetPlatform InPlatform)
        {
            return (
                (InPlatform == STTargetPlatform.Win64) ||
                (InPlatform == STTargetPlatform.Win32) ||
        ...
        static private bool GetPlatforms(ref List<STTargetPlatform> OutPlatforms, Func<STTargetPlatform, bool> Predicate, bool bCheckValidity = true)
        {
            OutPlatforms.Clear();
        ...
        /// <param name="bCheckValidity">If true, ensure it is a valid platform.</param>
        /// <returns>True if successful and platforms are found, false if not.</returns>
        static public bool GetAllPlatforms(ref List<STTargetPlatform> OutPlatforms, bool bCheckValidity = true)
        {
        ...
        /// <param name="bCheckValidity">If true, ensure it is a valid platform.</param>
        /// <returns>True if successful and platforms are found, false if not.</returns>
        static public bool GetAllDesktopPlatforms(ref List<STTargetPlatform> OutPlatforms, bool bCheckValidity = true)
        {
            return GetPlatforms(ref OutPlatforms, STBuildTool.IsDesktopPlatform, bCheckValidity);
        }
        ...
        /// <param name="bCheckValidity">If true, ensure it is a valid platform.</param>
        /// <returns>True if successful and platforms are found, false if not.</returns>
        static public bool GetAllEditorPlatforms(ref List<STTargetPlatform> OutPlatforms, bool bCheckValidity = true)
        {
            return GetPlatforms(ref OutPlatforms, STBuildTool.IsEditorPlatform, bCheckValidity);
        }
        ...
        /// <param name="bCheckValidity">If true, ensure it is a valid platform.</param>
        /// <returns>True if successful and platforms are found, false if not.</returns>
        static public bool GetAllServerPlatforms(ref List<STTargetPlatform> OutPlatforms, bool bCheckValidity = true)
        {
            return GetPlatforms(ref OutPlatforms, STBuildTool.IsServerPlatform, bCheckValidity);
        }
        ...
        static public bool IsValidConfiguration(STTargetConfiguration InConfiguration)
        {
            if (RunningRocket())
            {
                if (
        ...
        /// <returns>true if valid, false if not</returns>
        static public bool IsValidPlatform(STTargetPlatform InPlatform)
        {
            if (RunningRocket())
            {
        ...
        /// <param name="InPlatform"></param>
        /// <returns>true if valid, false if not</returns>
        static public bool IsValidDesktopPlatform(STTargetPlatform InPlatform)
        {
            ...
            if (UBTAssembly != null)
            {
                ...
                if (!bSkipBuildPlatforms)
                {
                    ...
                    foreach (var CheckType in AllTypes)
                    {
                        ...
                        {
                            if (Utils.ImplementsInterface<ISTBuildPlatform>(CheckType))
                            {
                                Log.TraceVerbose("    Registering build platform: {0}", CheckType.ToString());
                                var TempInst = (STBuildPlatform)(UBTAssembly.CreateInstance(CheckType.FullName, true));
                ...
                foreach (var CheckType in AllTypes)
                {
                    if (CheckType.IsClass && !CheckType.IsAbstract)
                    {
                        if (Utils.ImplementsInterface<ISTToolChain>(CheckType))
                        {
                            Log.TraceVerbose("    Registering tool chain    : {0}", CheckType.ToString());
                            var TempInst = (STToolChain)(UBTAssembly.CreateInstance(CheckType.FullName, true));
                            TempInst.RegisterToolChain();
                        }
                        else if (Utils.ImplementsInterface<ISTBuildDeploy>(CheckType))
                        {
                            Log.TraceVerbose("    Registering build deploy  : {0}", CheckType.ToString());
                            var TempInst = (STBuildDeploy)(UBTAssembly.CreateInstance(CheckType.FullName, true));
                            TempInst.RegisterBuildDeploy();
                        }
                        else if (CheckType.IsSubclassOf(typeof(STPlatformProjectGenerator)))
                        {
                ...
                foreach (var ProjType in ProjectGeneratorList)
                {
                    ...
                    STPlatformProjectGenerator TempInst = (STPlatformProjectGenerator)(UBTAssembly.CreateInstance(ProjType.FullName, true));
        ...
        private static bool ParseRocketCommandlineArg(string InArg, ref string OutGameName)
        {
            ...
            {
                if (BuildHostPlatform.Current.Platform == STTargetPlatform.Linux)
                {
        ...
        {
            List<STTargetPlatform> Platforms = new List<STTargetPlatform>();
                ...
                if (CurArgument.StartsWith("-"))
                {
                    if (CurArgument.StartsWith("-Platforms=", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        while (PlatformList.Length > 0)
                        {
                            ...
                            STTargetPlatform SpecifiedPlatform = STTargetPlatform.Unknown;
                            foreach (STTargetPlatform PlatformParam in Enum.GetValues(typeof(STTargetPlatform)))
                            {
                            ...
                            if (SpecifiedPlatform != STTargetPlatform.Unknown)
                            {
            ...
            foreach (STTargetPlatform platform in Platforms)
            {
                var BuildPlatform = STBuildPlatform.GetBuildPlatform(platform, true);
        ...
        public static ECompilationResult RunUBT(string[] Arguments)
        {
            ...
            STTargetPlatform ResetPlatform = STTargetPlatform.Unknown;
            STTargetConfiguration ResetConfiguration;
            STBuildTarget.ParsePlatformAndConfiguration(Arguments, out ResetPlatform, out ResetConfiguration);
            if (RunningRocket())
            {
                // Only Development and Shipping are supported for engine modules
                if (ResetConfiguration != STTargetConfiguration.Development && ResetConfiguration != STTargetConfiguration.Shipping)
                {
                    ResetConfiguration = STTargetConfiguration.Development;
                }
            }
            var BuildPlatform = STBuildPlatform.GetBuildPlatform(ResetPlatform);
            ...
            var ToolChain = STToolChain.GetPlatformToolChain(BuildPlatform.GetCPPTargetPlatform(ResetPlatform));
            ...
            try
            {
                ...
                if (CreateStub || (String.IsNullOrEmpty(Environment.GetEnvironmentVariable("uebp_LOCAL_ROOT")) && BuildHostPlatform.Current.Platform == STTargetPlatform.Mac))
                {
                ...
                var TargetDescs = new List<TargetDescriptor>();
                {
                    ...
                    {
                        TargetDescs.AddRange(STBuildTarget.ParseTargetCommandLine(TargetSetting));
                    }
                ...
                UBTMakefile UBTMakefile = null;
                {
                    ...
                    {													// @todo ubtmake: Won't catch multi-target cases as GPF always builds one target at a time for Intellisense
                        ...
                        {
                            STBuildTarget.CleanFile(UBTMakefilePath);
                        }
                    }

                    // Make sure the gather phase is executed if we're not actually building anything
                    if (ProjectFileGenerator.bGenerateProjectFiles || STBuildConfiguration.bGenerateManifest || STBuildConfiguration.bCleanProject)
                    {
                        STBuildTool.bIsGatheringBuild_Unsafe = true;
                    }
                    ...
                    if (STBuildTool.bIsAssemblingBuild_Unsafe && !STBuildTool.bIsGatheringBuild_Unsafe)
                    {
                        // @todo ubtmake: Mildly terrified of BuildConfiguration/STBuildConfiguration globals that were set during the Prepare phase but are not set now.  We may need to save/load all of these, otherwise
                    ...
                    // OK, after this point it is safe to access the STBuildTool.IsGatheringBuild and STBuildTool.IsAssemblingBuild properties.
                    // These properties will not be changing again during this session/
                    bIsSafeToCheckIfGatheringOrAssemblingBuild = true;
                }


                List<STBuildTarget> Targets;
                ...
                else
                {
                    ...
                    Targets = new List<STBuildTarget>();
                    foreach (var TargetDesc in TargetDescs)
                    {
                        var Target = STBuildTarget.CreateTarget(TargetDesc);
                        if ((Target == null) && (STBuildConfiguration.bCleanProject))
                        {
                            continue;
                        }
                        Targets.Add(Target);
                ...
                var TargetNameToUObjectModules = new Dictionary<string, List<SHTModuleInfo>>(StringComparer.InvariantCultureIgnoreCase);
                foreach (var Target in Targets)
                {
                    var TargetStartTime = DateTime.UtcNow;

                    // Check if we're currently running an editor and this is hot-reload from IDE
                    STBuildConfiguration.bHotReloadFromIDE = STBuildConfiguration.bAllowHotReloadFromIDE && !Target.bEditorRecompile && ShouldDoHotReload(Target);
                    ...
                    if (BuildConfiguration.bUseExperimentalFastDependencyScan &&
                        !(STBuildTool.IsGatheringBuild && !STBuildTool.IsAssemblingBuild))
                    {
                    ...
                    if (STBuildTool.IsGatheringBuild)
                    {
                        List<FileItem> TargetOutputItems;
                        List<SHTModuleInfo> TargetUObjectModules;
                        ...
                        if ((BuildConfiguration.bXGEExport && STBuildConfiguration.bGenerateManifest) || (!ProjectFileGenerator.bGenerateProjectFiles && !STBuildConfiguration.bGenerateManifest && !STBuildConfiguration.bCleanProject))
                        {
                        ...
                        Telemetry.SendEvent("TargetBuildStats.2",
                            ...
                            "CleanTarget", STBuildConfiguration.bCleanProject.ToString(),
                ...
                if (BuildResult == ECompilationResult.Succeeded &&
                    (
                        (BuildConfiguration.bXGEExport && STBuildConfiguration.bGenerateManifest) ||
                        (!GeneratingActionGraph && !ProjectFileGenerator.bGenerateProjectFiles && !STBuildConfiguration.bGenerateManifest && !STBuildConfiguration.bCleanProject && !STBuildConfiguration.bGenerateExternalFileList)
                    ))
                {
                    if (STBuildTool.IsGatheringBuild)
                    {
                    ...
                    if (STBuildTool.IsAssemblingBuild)
                    {
                        ...
                        if (BuildResult.Succeeded())
                        {
                            ...
                            Dictionary<STBuildTarget, List<FileItem>> TargetToOutdatedPrerequisitesMap;
                            ...
                            if (bSuccess)
                            {
                                foreach (STBuildTarget Target in Targets)
                                {
                                    ToolChain.PostBuildSync(Target);
                                }
                                if (ActionsToExecute.Count == 0 && STBuildConfiguration.bSkipLinkingWhenNothingToCompile)
                                {
        ...
        private static Thread CreateThreadForCachingCPPIncludes(Dictionary<STBuildTarget, List<FileItem>> TargetToOutdatedPrerequisitesMap)
        {
            return new Thread(new ThreadStart(() =>
            {
                ...
                foreach (var TargetAndOutdatedPrerequisites in TargetToOutdatedPrerequisitesMap)
                {
                    ...
                    foreach (var PrerequisiteItem in OutdatedPrerequisites)
                    {
                        ...
                        var FileBuildPlatform = STBuildPlatform.GetBuildPlatform(Target.GetTargetInfo().Platform);
        ...
        class UBTMakefile
        {
            ...
            public Dictionary<string, List<SHTModuleInfo>> TargetNameToUObjectModules;

            /** List of targets being built */
            public List<STBuildTarget> Targets;
        ...
        static UBTMakefile LoadUBTMakefile(List<TargetDescriptor> TargetDescs, out string ReasonNotLoaded)
        {
            ...
            if (UBTMakefileItem.bExists)
            {
                ...
                if (!bForceOutOfDate)
                {
                    ...
                    {
                        var MasterProjectRelativePath = STBuildTool.GetUProjectPath();
                ...
                if (!bForceOutOfDate)
                {
                    // Check to see if STBuildTool.exe was compiled more recently than the UBTMakefile
                    var STBuildToolTimestamp = new FileInfo(Assembly.GetExecutingAssembly().Location).LastWriteTime;
                    if (UBTMakefileItem.LastWriteTime.CompareTo(STBuildToolTimestamp) < 0)
                    {
                        // STBuildTool.exe was compiled more recently than the UBTMakefile
                        Log.TraceVerbose("Makefile is older than STBuildTool.exe, ignoring it");

                        ReasonNotLoaded = "STBuildTool.exe is newer";
                        bForceOutOfDate = true;
                    }
        ...
        public static string GetUBTMakefilePath(List<TargetDescriptor> TargetDescs)
        {
            ...
            else
            {
                ...
                if (STBuildTool.HasUProjectFile())
                {
                    ProjectIntermediatePath = Path.Combine(STBuildTool.GetUProjectPath(), BuildConfiguration.BaseIntermediatePath);
                }
