...
namespace STBuildTool
{
    ...
    public class ExternalExecution
    {
        ...
        {
            STTargetPlatform Platform = BuildHostPlatform.Current.Platform;
            string ExeExtension = STBuildPlatform.GetBuildPlatform(Platform).GetBinaryExtension(STBuildBinaryType.Executable);
        ...
        static VersionedBinary[] GetHeaderToolBinaries()
        {
            ...
            if (File.Exists(HeaderToolExe))
            {
                ...
                var DLLExtension = STBuildPlatform.GetBuildPlatform(Platform).GetBinaryExtension(STBuildBinaryType.DynamicLinkLibrary);
         ...
         * */
        private static bool AreGeneratedCodeFilesOutOfDate(List<SHTModuleInfo> UObjectModules)
        {
            ...
            foreach (var Module in UObjectModules)
            {
                ...
                if (STBuildTool.RunningRocket())
                {
                    ...
                    if (!Utils.IsFileUnderDirectory(Module.ModuleDirectory, STBuildTool.GetUProjectPath()))
                    {
        ...
        private static void UpdateDirectoryTimestamps(List<SHTModuleInfo> UObjectModules)
        {
            foreach (var Module in UObjectModules)
            {
