...
namespace STBuildTool
{
    ...
    public abstract class ModuleRules
    {
        ...
        public List<STBuildFramework> PublicAdditionalFrameworks = new List<STBuildFramework>();

        /// List of addition resources that should be copied to the app bundle for Mac or iOS
        public List<STBuildBundleResource> AdditionalBundleResources = new List<STBuildBundleResource>();
        ...
        {
            /*
            if (STBuildTool.RunningRocket() == false || Target.Type == TargetRules.TargetType.Game)
            {
        ...
        /// <summary>
        /// Setup this module for PhysX/APEX support (based on the settings in STBuildConfiguration)
        /// </summary>
        public void SetupModulePhysXAPEXSupport(TargetInfo Target)
        {
            if (STBuildConfiguration.bCompilePhysX == true)
            {
                ...
                if (STBuildConfiguration.bCompileAPEX == true)
                {
            ...
            if (STBuildConfiguration.bRuntimePhysicsCooking == true)
            {
        ...
        /// <summary>
        /// Setup this module for Box2D support (based on the settings in STBuildConfiguration)
        /// </summary>
        public void SetupModuleBox2DSupport(TargetInfo Target)
        {
            ...
            bSupported = bSupported && STBuildConfiguration.bCompileBox2D;
    ...
    public abstract class TargetRules
    {
        ...
        public virtual bool ShouldCompileMonolithic(STTargetPlatform InPlatform, STTargetConfiguration InConfiguration)
        {
            ...
            if (IsEditorType(Type) == false)
            {
                ...
                if (STBuildTool.CommandLineContains("-modular") == false)
                {
                    return true;
                }
            }
        ...
        public virtual bool GetSupportedPlatforms(ref List<STTargetPlatform> OutPlatforms)
        {
            if (Type == TargetType.Program)
            {
                ...
                return STBuildTool.GetAllDesktopPlatforms(ref OutPlatforms, false);
            }
            else if (IsEditorType(Type))
            {
                return STBuildTool.GetAllEditorPlatforms(ref OutPlatforms, false);
            }
            else if (TargetRules.IsGameType(Type))
            {
                // By default all games support all platforms
                return STBuildTool.GetAllPlatforms(ref OutPlatforms, false);
            }
    ...
    class RulesCompiler
    {
            ...
            if (DirInfo.Exists)
            {
                ...
                if (!bFoundModuleRulesFile)
                {
                    ...
                    foreach (DirectoryInfo SubDirInfo in DirInfo.GetDirectories())
                    {
                        ...
                        {
                            Console.WriteLine("WARNING: STBuildTool found an Intermediate folder while looking for rules '{0}'.  It should only ever be searching under 'Source' folders -- an Intermediate folder is unexpected and will greatly decrease iteration times!", SubDirInfo.FullName);
                        }
        ...
        private static void ConditionallyCompileAndLoadRulesAssembly()
        {
            ...
            if (RulesAssembly == null)
            {
                ...
                if (STBuildTool.HasUProjectFile())
                {
                    ...
                    var ProjectSourceDirectory = Path.Combine(STBuildTool.GetUProjectPath(), "Source");
                ...
                string BaseIntermediatePath = STBuildTool.HasUProjectFile() ?
                    Path.Combine(STBuildTool.GetUProjectPath(), BuildConfiguration.BaseIntermediateFolder) : BuildConfiguration.BaseIntermediatePath;
        ...
        public static bool IsRocketProjectModule(string InModuleName)
        {
            ...
            return (Utils.IsFileUnderDirectory(Filename, STBuildTool.GetUProjectPath()));
        }
        ...
        public static ModuleRules CreateModuleRules(string ModuleName, TargetInfo Target, out string ModuleFileName)
        {
            ...
            STTargetPlatform LocalPlatform = Target.Platform;
            STTargetConfiguration LocalConfiguration = Target.Configuration;
            ...
            if (STBuildTool.RunningRocket())
            {
                ...
                if (!Utils.IsFileUnderDirectory(ModuleFileName, FullUProjectPath))
                {
                    ...
                    if (LocalConfiguration != STTargetConfiguration.Development && LocalConfiguration != STTargetConfiguration.Shipping)
                    {
                        LocalConfiguration = STTargetConfiguration.Development;
                    }
                }
            }
            ...
            if (RulesObjectType == null)
            {
                ...
                RulesObjectType = RulesAssembly.GetType("STBuildTool.Rules." + ModuleName);
            }
        ...
        public static void AddDefaultIncludePathsToModuleRules(STBuildTarget BuildTarget, string InModuleName, string InModuleFilename, string InModuleFileRelativeToEngineDirectory, bool IsGameModule, ref ModuleRules RulesObject)
        {
            ...
            string IntermediateDirectory = STBuildModuleCPP.GetGeneratedCodeDirectoryForModule(BuildTarget, Path.GetDirectoryName(InModuleFilename), InModuleName);
