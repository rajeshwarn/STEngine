...
namespace STBuildTool
{
    public abstract class MSBuildProjectFile : ProjectFile
    {
        ...
        /// to Visual Studio
        static public readonly string StubProjectConfigurationName = "BuiltWithSTBuildTool";
    ...
    public class VCProjectFile : MSBuildProjectFile
    {
        ...
        public override bool WriteProjectFile(List<STTargetPlatform> InPlatforms, List<STTargetConfiguration> InConfigurations)
        {
            ...
            var VCIncludeSearchPaths = new StringBuilder();
            {
                ...
                if (InPlatforms.Contains(STTargetPlatform.Win64))
                {
                    VCIncludeSearchPaths.Append(VCToolChain.GetVCIncludePaths(CPPTargetPlatform.Win64) + ";");
                }
                else if (InPlatforms.Contains(STTargetPlatform.Win32))
                {
            ...
            bool bGenerateUserFileContent = STPlatformProjectGenerator.PlatformRequiresVSUserFileGeneration(InPlatforms, InConfigurations);
            ...
            if (IsStubProject)
            {
                ProjectConfigAndTargetCombination StubCombination = new ProjectConfigAndTargetCombination(STTargetPlatform.Unknown, STTargetConfiguration.Unknown, StubProjectPlatformName, StubProjectConfigurationName, null);
            ...
            else
            {
                ...
                foreach (var Configuration in InConfigurations)
                {
                    ...
                    if (STBuildTool.IsValidConfiguration(Configuration) == false)
                    {
                        continue;
                    }
                    foreach (var Platform in InPlatforms)
                    {
                        if (STBuildTool.IsValidPlatform(Platform) == false)
                        {
                            continue;
                        }
                        var BuildPlatform = STBuildPlatform.GetBuildPlatform(Platform, true);
            ...
            var ProjectPlatforms = new List<STTargetPlatform>();
            ...
            if (!IsStubProject)
            {
                ...
                {
                    STPlatformProjectGenerator ProjGenerator = STPlatformProjectGenerator.GetPlatformProjectGenerator(Platform, true);
                    if (ProjGenerator != null && ProjGenerator.HasVisualStudioSupport(Platform, STTargetConfiguration.Development))
                    {
            ...
            if (STBuildTool.RunningRocket() && !IsStubProject)
            {
        ...
        {
            STTargetPlatform Platform = Combination.Platform;
            STTargetConfiguration Configuration = Combination.Configuration;

            STPlatformProjectGenerator ProjGenerator = STPlatformProjectGenerator.GetPlatformProjectGenerator(Platform, true);
            if (((ProjGenerator == null) && (Platform != STTargetPlatform.Unknown)))
            {
                return;
            }
...

            {
                ...
                else
                {
                    ...
                    {
                        if (STBuildTool.HasUProjectFile() && Utils.IsFileUnderDirectory(TargetFilePath, STBuildTool.GetUProjectPath()))
                        {
                            RootDirectory = Path.GetFullPath(STBuildTool.GetUProjectPath());
                        }
                        else
                        {
                    ...
                    if (Configuration != STTargetConfiguration.Development && (Configuration != STTargetConfiguration.DebugGame || bShouldCompileMonolithic))
                    {
                        NMakePath += "-" + UBTPlatformName + "-" + UBTConfigurationName;
                    }
                    NMakePath += BuildPlatform.GetActiveArchitecture();
                    NMakePath += BuildPlatform.GetBinaryExtension(STBuildBinaryType.Executable);
                    NMakePath = (BuildPlatform as STBuildPlatform).ModifyNMakeOutput(NMakePath);
                        ...
                        (TargetRules.IsEditorType(TargetRulesObject.Type) == false))
                    {
                        ...
                        STPlatformProjectGenerator.GenerateGamePlatformSpecificProperties(Platform, Configuration, TargetRulesObject.Type, VCProjectFileContent, RootDirectory, TargetFilePath);
                    }
                    ...
                    if (IsForeignProject)
                    {
                        VCProjectFileContent.Append(" " + UProjectPath + (STBuildTool.RunningRocket() ? " -rocket" : ""));
                    }
                ...
                if (VCUserFileContent != null && Combination.ProjectTarget != null)
                {
                    ...
                    if ((Platform == STTargetPlatform.Win32) || (Platform == STTargetPlatform.Win64))
                    {
                        ...
                        if (TargetRulesObject.Type != TargetRules.TargetType.Game)
                        {
                            ...
                            if (Configuration == STTargetConfiguration.Debug || Configuration == STTargetConfiguration.DebugGame)
                            {
                                DebugOptions += " -debug";
                            }
                            else if (Configuration == STTargetConfiguration.Shipping)
                            {
