...
namespace STBuildTool
{
    public abstract class Utils
    {
        ...
        public static List<string> MakeListOfUnsupportedPlatforms(List<STTargetPlatform> SupportedPlatforms)
        {
            ...
            var OtherPlatformNameStrings = new List<string>();
            {
                ...
                List<STPlatformGroup> SupportedGroups = new List<STPlatformGroup>();
                foreach (STPlatformGroup Group in Enum.GetValues(typeof(STPlatformGroup)))
                {
                    // get the list of platforms registered to this group, if any
                    List<STTargetPlatform> Platforms = STBuildPlatform.GetPlatformsInGroup(Group);
                    if (Platforms != null)
                    {
                        // loop over each one
                ...
                foreach (STTargetPlatform CurPlatform in Enum.GetValues(typeof(STTargetPlatform)))
                {
                    if (CurPlatform != STTargetPlatform.Unknown)
                    {
