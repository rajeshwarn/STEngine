...
namespace STBuildTool
{
    ...
    public class XcodeProjectTarget
    {
        ...
        public XcodeProjectTarget(string InDisplayName, string InTargetName, XcodeTargetType InType, string InTargetFilePath, string InProductName = "", STTargetPlatform InTargetPlatform = STTargetPlatform.Mac, bool bInIsMacOnly = false, List<XcodeTargetDependency> InDependencies = null, bool bHasPlist = false, List<XcodeFrameworkRef> InFrameworks = null)
        {
        ...
        public STTargetPlatform TargetPlatform;	//Mac, IOS
    ...
    class XcodeProjectFileGenerator : ProjectFileGenerator
    {
        ...
        private void AppendTarget(ref StringBuilder Contents, XcodeProjectTarget Target)
        {
            ...
            {
                string UProjectPath = "";
                if (STBuildTool.HasUProjectFile() && !string.IsNullOrEmpty(Target.TargetFilePath) && Utils.IsFileUnderDirectory(Target.TargetFilePath, STBuildTool.GetUProjectPath()))
                {
                    if (MasterProjectRelativePath == STBuildTool.GetUProjectPath())
                    {
                        UProjectPath = " " + "\\\"$(PROJECT_DIR)/" + Path.GetFileName(STBuildTool.GetUProjectFile()) + "\\\"";
                    }
                    else
                    {
                        UProjectPath = " " + "\\\"" + STBuildTool.GetUProjectFile() + "\\\"";
                    }
                }
        ...
        private void AppendMacBuildConfig(ref StringBuilder Contents, string ConfigName, string ConfigGuid, bool bIsMacOnly)
        {
            ...
            {
                ISTToolChain Toolchain = STToolChain.GetPlatformToolChain(CPPTargetPlatform.IOS);
            ...
            string EngineRelative, string GamePath, bool bIsUE4Game, bool IsAGame, bool bIsUE4Client)
        {
            ...
            {
                if (Target.TargetPlatform == STTargetPlatform.Mac)
                {
        ...
        {
            List<string> GameFolders = STBuildTarget.DiscoverAllGameFolders();
            ...
            ref List<string> SystemIncludeDirectories, ref List<string> PreprocessorDefinitions)
        {
            ...
            if (!bGeneratingRocketProjectFiles)
            {
                ...
                string ProjectPath = System.IO.Path.Combine(System.IO.Path.Combine(EngineRelativePath, "Source"), "Programs", "STBuildTool", "STBuildTool_Mono.csproj");
        ...
        private void WriteSchemes(string XcodeProjectPath, List<XcodeProjectTarget> ProjectTargets)
        {
            ...
            foreach (var Target in ProjectTargets)
            {
                ...
                if (STBuildTool.HasUProjectFile() && !string.IsNullOrEmpty(Target.TargetFilePath) && Utils.IsFileUnderDirectory(Target.TargetFilePath, STBuildTool.GetUProjectPath()))
                {
                    ...
                    {
                        WriteSchemeFile(XcodeProjectPath, Target, ".app", "UE4Editor", new List<string>(new string[] { "&quot;" + STBuildTool.GetUProjectFile() + "&quot;" }));
                    }
                    else
                    {
                        string ProjectBinariesDir = STBuildTool.GetUProjectPath() + "/Binaries/Mac/";
                        WriteSchemeFile(XcodeProjectPath, Target, ".app", ProjectBinariesDir + Target.TargetName, new List<string>(new string[] { "&quot;" + STBuildTool.GetUProjectFile() + "&quot;" }));
                    }
                }
                ...
                else if (TargetsThatNeedApp.Contains(Target.TargetName) || (Target.TargetPlatform == STTargetPlatform.IOS))
                {
        ...
        private void WriteProject(StringBuilder XcodeProjectFileContent, List<XcodeProjectTarget> ProjectTargets)
        {
            ...
            string PathBranch = (BuildHostPlatform.Current.Platform != STTargetPlatform.Mac) ? "/Engine/Intermediate/IOS" : "";
            ...
            {
                XcodeProjectPath = STBuildTool.GetUProjectPath() + "/" + MasterProjectName + ".xcodeproj";
            }
